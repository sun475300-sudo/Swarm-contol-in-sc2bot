        ...
        try:
            ...
            if is_defenseless:
                ...
                if nearby_workers.exists:
                    ...
                    # 최대 동원 가능 수 = 전체 일꾼 - 최소 유지 일꾼 수
                    max_workers_to_pull = min(
                        max_pullable_workers,  # 최대 동원 가능 수 (최소 유지 수 보장)
                        max(int(worker_count * retreat_threshold), 1),  # 최소 1기
                        min(10, len(workers_list)),  # 최대 10기
                    )
                    
                    # 절체절명 상황에서도 최소 일꾼은 보존
                    if max_workers_to_pull <= 0:
                        # 최소 일꾼 수를 보존할 수 없으면 동원하지 않음
                        return
