...
- Enemy (10): 
  ...
  - Scout Coverage (0-1)
  - **NEW**: Enemy Main Distance (0-1, normalized) - 적 본진까지 거리
  - **NEW**: Enemy Expansion Count (0-1, normalized) - 적 확장 개수
  - **NEW**: Enemy Resource Estimate (0-1, normalized) - 적 자원 추정
  - **NEW**: Enemy Upgrade Count (0-1, normalized) - 적 업그레이드 개수
  - **NEW**: Enemy Air/Ground Ratio (0-1) - 적 공중/지상 비율
...
**3. `wicked_zerg_bot_pro.py` - 모델 생성 업데이트**:
...
- ? 프로게이머 전략(이병렬의 맹독충 드랍 등) 제대로 흡수 가능
...
- ? `learning_status.json` 저장 시 Atomic write 적용
...
1. **`local_training/wicked_zerg_bot_pro.py`**
   ...
   - 적 위치, 자원, 확장, 업그레이드, 공중/지상 비율 정보 추가
   - `ZergNet(input_size=15)` 명시적 설정
   - 기본 상태 반환: 10개 → 15개
...
6. **`local_training/combat_manager.py`**
   - `closer_than` API 추가 적용 (3곳)

### 문서화
1. **`설명서/ENHANCED_LEARNING_SYSTEM_REPORT.md`** (신규)
   - 향상된 학습 시스템 구현 보고서

2. **`설명서/COMPREHENSIVE_FIXES_FINAL_REPORT.md`** (신규)
   - 종합 문제점 해결 최종 보고서
...
### 학습 효율 향상
- **15차원 입력**: 적 위치, 자원, 확장, 업그레이드, 공중/지상 비율 정보 포함
- **프로게이머 전략 흡수**: 이병렬의 맹독충 드랍 타이밍 등 구체적 전략 학습 가능
- **상황 인식 학습**: 적의 상황에 따른 유연한 대응 학습
- **맥락 인식 의사결정**: 적 위치와 자원에 따른 전략 선택 가능
...
2. **15차원 신경망 입력**
   - 적 위치, 자원, 확장, 업그레이드, 공중/지상 비율 정보 포함
   - 프로게이머 전략 학습 가능
   - 맥락 인식 의사결정 가능
